use crate::*;

pub struct Sun {
    pub date: DateTime,
}

impl std::fmt::Display for Sun {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}",
        self.date,
        )
    }
}

impl Sun {

    pub fn new_day() -> Sun {
        Sun {
            date: new_day(2000, Month::JANUARY, 1),
        }
    }

    /// Get a characteristic moon at a given DateTime and observer lat/lon
    /// Assumes a 1m observation height
    pub fn set(&mut self, date: DateTime) {// obs_lat: f64, obs_lon: f64) {
        self.date = date;
        //let obs_elev = 1.0; // meters
        // Julian dates
        let jd = julian_day(&self.date);
        let jde = julian_ephemeris_day(jd, self.date.delta_t);
        let jc = julian_century(jd);
        let jce = julian_ephemeris_century(jde);
        let jme = jce / 10.0;

        let l0 = l0_rad(jme);
        let l1 = l1_rad(jme);
        let l2 = l2_rad(jme);
        let l3 = l3_rad(jme);
        let l4 = l4_rad(jme);
        let l5 = l5_rad(jme);

        println!("jd: {}", jd);
        println!("l0: {}", l0);
        println!("l1: {}", l1);
        println!("l2: {}", l2);
        println!("l3: {}", l3);
        println!("l4: {}", l4);
        println!("l5: {}", l5);
    }
}

fn l0_rad(jme: f64) -> f64 {
    let values: Vec<(f64, f64, f64)> = vec![
        (175347046.0, 0.0, 0.0),
        (3341656.0, 4.6692568, 6283.07585),
        (34894.0, 4.6261, 12566.1517),
        (3497.0, 2.7441, 5753.3849),
        (3418.0, 2.8289, 3.5231),
        (3136.0, 3.6277, 77713.7715),
        (2676.0, 4.4181, 7860.4194),
        (2343.0, 6.1352, 3930.2097),
        (1324.0, 0.7425, 11506.7698),
        (1273.0, 2.0371, 529.691),
        (1199.0, 1.1096, 1577.3435),
        (990.0, 5.233, 5884.927),
        (902.0, 2.045, 26.298),
        (857.0, 3.508, 398.149),
        (780.0, 1.179, 5223.694),
        (753.0, 2.533, 5507.553),
        (505.0, 4.583, 18849.228),
        (492.0, 4.205, 775.523),
        (357.0, 2.92, 0.067),
        (317.0, 5.849, 11790.629),
        (284.0, 1.899, 796.298),
        (271.0, 0.315, 10977.079),
        (243.0, 0.345, 5486.778),
        (206.0, 4.806, 2544.314),
        (205.0, 1.869, 5573.143),
        (202.0, 2.458, 6069.777),
        (156.0, 0.833, 213.299),
        (132.0, 3.411, 2942.463),
        (126.0, 1.083, 20.775),
        (115.0, 0.645, 0.98),
        (103.0, 0.636, 4694.003),
        (102.0, 0.976, 15720.839),
        (102.0, 4.267, 7.114),
        (99.0, 6.21, 2146.17),
        (98.0, 0.68, 155.42),
        (86.0, 5.98, 161000.69),
        (85.0, 1.3, 6275.96),
        (85.0, 3.67, 71430.7),
        (80.0, 1.81, 17260.15),
        (79.0, 3.04, 12036.46),
        (75.0, 1.76, 5088.63),
        (74.0, 3.5, 3154.69),
        (74.0, 4.68, 801.82),
        (70.0, 0.83, 9437.76),
        (62.0, 3.98, 8827.39),
        (61.0, 1.82, 7084.9),
        (57.0, 2.78, 6286.6),
        (56.0, 4.39, 14143.5),
        (56.0, 3.47, 6279.55),
        (52.0, 0.19, 12139.55),
        (52.0, 1.33, 1748.02),
        (51.0, 0.28, 5856.48),
        (49.0, 0.49, 1194.45),
        (41.0, 5.37, 8429.24),
        (41.0, 2.4, 19651.05),
        (39.0, 6.17, 10447.39),
        (37.0, 6.04, 10213.29),
        (37.0, 2.57, 1059.38),
        (36.0, 1.71, 2352.87),
        (36.0, 1.78, 6812.77),
        (33.0, 0.59, 17789.85),
        (30.0, 0.44, 83996.85),
        (30.0, 2.74, 1349.87),
        (25.0, 3.16, 4690.48)];

    let mut l = 0.0;
    for v in values.iter() {
        l += v.0 * (v.1 + v.2 * jme).cos();
    }
    l
}

fn l1_rad(jme: f64) -> f64 {
    let values: Vec<(f64, f64, f64)> = vec![
        (628331966747.0, 0.0, 0.0),
        (206059.0, 2.678235, 6283.07585),
        (4303.0, 2.6351, 12566.1517),
        (425.0, 1.59, 3.523),
        (119.0, 5.796, 26.298),
        (109.0, 2.966, 1577.344),
        (93.0, 2.59, 18849.23),
        (72.0, 1.14, 529.69),
        (68.0, 1.87, 398.15),
        (67.0, 4.41, 5507.55),
        (59.0, 2.89, 5223.69),
        (56.0, 2.17, 155.42),
        (45.0, 0.4, 796.3),
        (36.0, 0.47, 775.52),
        (29.0, 2.65, 7.11),
        (21.0, 5.34, 0.98),
        (19.0, 1.85, 5486.78),
        (19.0, 4.97, 213.3),
        (17.0, 2.99, 6275.96),
        (16.0, 0.03, 2544.31),
        (16.0, 1.43, 2146.17),
        (15.0, 1.21, 10977.08),
        (12.0, 2.83, 1748.02),
        (12.0, 3.26, 5088.63),
        (12.0, 5.27, 1194.45),
        (12.0, 2.08, 4694.0),
        (11.0, 0.77, 553.57),
        (10.0, 1.3, 6286.6),
        (10.0, 4.24, 1349.87),
        (9.0, 2.7, 242.73),
        (9.0, 5.64, 951.72),
        (8.0, 5.3, 2352.87),
        (6.0, 2.65, 9437.76),
        (6.0, 4.67, 4690.48)];
    
    let mut l = 0.0;
    for v in values.iter() {
        l += v.0 * (v.1 + v.2 * jme).cos();
    }
    l
}

fn l2_rad(jme: f64) -> f64 {
    let values: Vec<(f64, f64, f64)> = vec![
        (52919.0, 0.0, 0.0),
        (8720.0, 1.0721, 6283.0758),
        (309.0, 0.867, 12566.152),
        (27.0, 0.05, 3.52),
        (16.0, 5.19, 26.3),
        (16.0, 3.68, 155.42),
        (10.0, 0.76, 18849.23),
        (9.0, 2.06, 77713.77),
        (7.0, 0.83, 775.52),
        (5.0, 4.66, 1577.34),
        (4.0, 1.03, 7.11),
        (4.0, 3.44, 5573.14),
        (3.0, 5.14, 796.3),
        (3.0, 6.05, 5507.55),
        (3.0, 1.19, 242.73),
        (3.0, 6.12, 529.69),
        (3.0, 0.31, 398.15),
        (3.0, 2.28, 553.57),
        (2.0, 4.38, 5223.69),
        (2.0, 3.75, 0.98)];
    
    let mut l = 0.0;
    for v in values.iter() {
        l += v.0 * (v.1 + v.2 * jme).cos();
    }
    l
}

fn l3_rad(jme: f64) -> f64 {
    let values: Vec<(f64, f64, f64)> = vec![
        (289.0, 5.844, 6283.076),
        (35.0, 0.0, 0.0),
        (17.0, 5.49, 12566.15),
        (3.0, 5.2, 155.42),
        (1.0, 4.72, 3.52),
        (1.0, 5.3, 18849.23),
        (1.0, 5.97, 242.73)];
    
    let mut l = 0.0;
    for v in values.iter() {
        l += v.0 * (v.1 + v.2 * jme).cos();
    }
    l
}

fn l4_rad(jme: f64) -> f64 {
    let values: Vec<(f64, f64, f64)> = vec![
        (114.0, 3.142, 0.0),
        (8.0, 4.13, 6283.08),
        (1.0, 3.84, 12566.15)];
    
    let mut l = 0.0;
    for v in values.iter() {
        l += v.0 * (v.1 + v.2 * jme).cos();
    }
    l
}

fn l5_rad(jme: f64) -> f64 {    
    1.0 * (3.14 + 0.0 * jme).cos()
}